...
bool palmStabilityCallBack(std_srvs::Empty::Request&, std_srvs::Empty::Response&)
{
    ...
    while (abs(test_send.position[0])> 5)
    {
   	...
   	//palmMotorCurrentControl(1, motor_current[1]);
	stringstream msg1;
	msg1 << "current is: " << realtime_control_signal[0] << "'!";
	LogInfo(msg1.str());
   	ros::spinOnce();
    //palmMotorCurrentControl(1,50);
...
int main(int argc, char** argv)
{
	...
	ros::Subscriber palm_control_signal_sub = n.subscribe("palm_control_signal", 10, palm_control_signal_call_back);	
