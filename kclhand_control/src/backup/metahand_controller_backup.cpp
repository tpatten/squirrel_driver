...
const unsigned int maximumCurrent = 250;
...
bool motorRotationWithVelocity(unsigned short motorTotalNumber)
{
    ...
    while((handCurrentChecker() == true) && (handConfigurationCheck(test_send) == true))
    {
	 	...
	 	for(index=2; index<motorTotalNumber; index++)
	 	{
		    ...
		    if(motor_state == 1)
			{
				...
				if((lResult = Demo(&ulErrorCode, 2000*motor_direction[index]))!=MMC_SUCCESS)
				{
					return lResult;
				}
			}
	...
	while (handConfigurationCheck(test_send) == true)
	{
...
/*
		...
		{
			if (currentValue >= setHoldingCurrent)
			{
				if((lResult = CurrentDemo(&ulErrorCode, setHoldingCurrent*motor_direction[index]))!=MMC_SUCCESS)
				{
					return lResult;
