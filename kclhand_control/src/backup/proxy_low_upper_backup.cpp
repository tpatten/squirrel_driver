...
void KCLHandController::impedanceSignalCB(const std_msgs::Float64MultiArray::ConstPtr &msg)
{
 ...
 for (size_t i = 2; i < NUM_JOINTS; i++)
  ...
  impedance_motor_value.position[5] = joints_[2].motorVelocity();
  impedance_motor_value.position[6] = joints_[3].motorVelocity();
  impedance_motor_value.position[7] = joints_[4].motorVelocity();
...
bool KCLHandController::handImpedanceController(float rel_current_limit)
{
  ...
  while(1)
  {    
    bool succeeded = impedanceMoveFingers(rel_current_limit, all_target_positions_reached);
    rate.sleep();  
    if (!realtimeImpedanceFlag()) 
    {
      for(int i = 0; i < NUM_JOINTS; i++)
