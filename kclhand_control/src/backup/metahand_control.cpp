...
bool handConfigurationCheck(sensor_msgs::JointState test_send);
void disableMotor();
void haltPalmJoint(unsigned short motorTotalNumber, float configurationData[]);
...
float upper_sphere_central_grasp_angle[5] = {-40,-40,-60,-45,-60};
...
float handConfigurationUpperLimit[5] ={10,10, 25, 25, 25};
...
const unsigned int maximumCurrent = 200;
...
bool DemoProfileVelocityMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode, long motor_speed)
{
	...
	else
	{
			...
			if (VCS_GetCurrentIs(p_DeviceHandle, p_usNodeId, &temp_current, &p_rlErrorCode) != 0)
			{
				...
				if (temp_current > maximumCurrent)
				{
					targetvelocity = maximumCurrent;
...
bool closeFingerCallBack(kclhand_control::graspCurrent::Request &req, kclhand_control::graspCurrent::Response&)
{
    ...
    if (req.graspCurrentLimit >1)
    {
    	graspForce = maximumCurrent;
    }

    else if ((req.graspCurrentLimit>=0) && (req.graspCurrentLimit<=1))
    {
    	graspForce = maximumCurrent * req.graspCurrentLimit;
    }

    else 
    {
    	ROS_INFO("Wrong input; Please anter the value between 0 and 1");
    	return true;
    }
