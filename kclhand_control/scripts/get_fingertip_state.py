...
class kclhand_fingertip_state(kclhand.HandFK):
...
class send_joint_state(object):
    ...
    def __init__(self):
        ...
        self.hand_joint_value = [0,0,0,0,0]

        rospy.Subscriber("sensorToForwardKinematics", JointState, self.sensor_call_back)
        fingertip_data = rospy.Publisher('fingertip_state', Float64MultiArray, queue_size=10)
        joint_displacement_array = rospy.Publisher('post_joint_value', JointState, queue_size=10)

        data_send = Float64MultiArray()
        displacement_send = JointState()

        rospy.init_node('get_fingertip_state', anonymous=True)
        ...
        while not rospy.is_shutdown():
                ...
                displacement_send.position = new_fingertip_state.get_joint_displacement()
    ...
    def sensor_call_back(self,msg):
        joint_value_from_sensor = JointState()
        self.hand_joint_value = msg.position
