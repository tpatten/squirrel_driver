...
class Controller(Rotation, Palm_solver):
    ...
    ### Inertial matrix D(q) and gravity effect g(q) of the closed chain model
        ...
        self.beta_q = np.column_stack((np.eye(4), np.zeros((4, 2))))
        ...
        self.rho = np.dot(np.linalg.inv(self.gamma_q), np.concatenate((np.zeros((2, 4)), np.eye(4))))
    ...
    def output(self, signals, dot_signals, desired_signals):
        ...
        self.K = np.array([[self.k1, 0.0, 0.0, 0.0, self.k2, 0.0, 0.0, 0.0],
                           [0.0, self.k1, 0.0, 0.0, 0.0, self.k2, 0.0, 0.0],
                           [0.0, 0.0, self.k1, 0.0, 0.0, 0.0, self.k2, 0.0],
                           [0.0, 0.0, 0.0, self.k1, 0.0, 0.0, 0.0, self.k2]])
...
print a_controller.output([1.33, 2.11, 1.5, 1.5], [-0.1, -0.1, -0.1, -0.1], [3.5, -0.8, 1.5, 1.5])
